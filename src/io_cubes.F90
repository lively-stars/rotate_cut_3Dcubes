...
!   Read binary 
  ...
  subroutine read_cube_bin(filename, nx, ny, nz, outarr)
   ...
   open (unit = 1, file= filename, form='unformatted', access = 'stream', status ='old')
       do i = 1, Nx
         do j = 1, Ny
           do k = 1, Nz
             read(1) outarr(i,j,k)
...
! --------------- READ FITS files + fits routines ---------------------------------
  ...
  subroutine read_cube_fits(filename, nx, ny, nz, outarr)
   ...
   integer, intent(in) :: nx, ny, nz
   real(kind=4), intent(out):: outarr(nx, nz, ny)
   character(*) :: filename
   integer i, j, k
   ...
   print*, ' reading imaged finished ' 
       do i = 1, Nx
         do j = 1, Ny
           do k = 1, Nz
           outarr(i,k,j) = bufffits(i,j,k)
           end do
         end do
       end do
   return 
...
!  cases where one is not interested in the contents.
      ...
      subroutine readimage(filename, nx, ny, nz)
