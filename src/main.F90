...
! get cube dimensions :
  ...
  elseif (ncread) then  
    filename =  trim(folder)//'cube_small.'//trim(filenumber)//'.nc'

    call open_nc_cube(ncid, filename, nx, ny, nz, dx, dy, dz, time, ier )
    print*,' opened nc-file, read params:'
    print*,' Nx, Ny, Nz = ', Nx, Ny, Nz
...
#ifdef FITS
     print*, '    '
     print*,' so far everything is set right, going to read fits files'
...
!----- check the orientation of the  vertical dimension of the cube!
   ...
   if (T(1,1,1) .gt. T(1,1,nz)) then 
     print*, ' cube is upside-down we flip it!' 
...
! z-grid needs to be set for the cube as it is read in:
   ...
   print*, ' zgird start = ', zgrid(1)
   print*, ' zgrid end =', zgrid(Nzcut) 
...
! --- if there was no rotation, then taur already holds the Rosseland tau
       do i = 1, Nx
          do k = 1, Ny
!    
           do j = 1, Nzcut 
...
! here we map column mass instead of rho
 ...
 print*,' nothing was changed, if atlas is set, writing the strucutre now' 

 endif ! --- if no calculations are needed 

!-------------------------------------------------------------------------!
!-------------------------------------------------------------------------!
! ---             Finally , PRINT OUT the results ------------------------!

!if the structure for the atlas calculations needs to be writen, we need rhox = integral (rho )dz 


     if (atlas) then
! atlas expects the atmosphere in this format:
! 
!--- rhox, t, p, xne,  abross, accrad, vturb,
! ----  ---------------------------------------------- 

! ---- need to use tempa to add 4 columns of zeros
!   use size to split structure into several files


       tempt = 0.0  
       Nzz = Nz
!--- IF NO CALCULATIONS WHERE DONE : 
       ...
       if (nocalc) then 
         ...
         do i = 1, Nx
          do k = 1,  Ny
            ...
            call integ(zgrid,tempr,tempp,Nzz,(tempr(1)*zgrid(1)))
            rho(i,k,1:Nzz) = tempp(1:Nzz)
          end do
         end do
...
! --- end of write out 
       ...
       else if (gettaug) then 
! ---- new approach 05.01.2023 -> first get column mass then map on tau grid 
...
!          do k = 1, Ny
!            do j = 1, Nzz
!             tempr(j) = outrho(i,k, j)
!            end do
!
!            call integ(zgrid,tempr,tempp,Nzz,(tempr(1)*zgrid(1)))
!            outrho(i,k,1:Nzz) = tempp(1:Nzz)
!          end do
!         end do
...
! ---- this is when the cube was rotated 
...
112       format(1PE14.7,0PF9.1,1P8E11.3,0PF12.3,F9.3)
