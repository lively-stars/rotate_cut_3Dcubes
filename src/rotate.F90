...
!--- over the x-y plan
      ...
      do i = 1, Nx
        do k = 1, Ny
           do j = 1, Nzcut 
             ...
             newT(i,k,j)  = ((T(xn, k, z_l)) * (1.0d0-x_f) + x_f*(T(xnp, k,  z_l))) *(1.0d0- z_f)
             newT(i,k,j)  = newT(i,k,j) +  z_f*((T(xn, k, z_l+1))*(1.0d0-x_f)+ x_f*(T(xnp, k,z_l+1)))

             newP(i,k,j)  = ((P(xn, k, z_l)) * (1.0d0-x_f) + x_f*(P(xnp, k,  z_l))) *(1.0d0- z_f)
             newP(i,k,j)  = newP(i,k,j) +  z_f*((P(xn, k, z_l+1))*(1.0d0-x_f)+ x_f*(P(xnp, k,z_l+1)))

             newrho(i,k,j)  = ((rho(xn, k, z_l)) * (1.0d0-x_f) + x_f*(rho(xnp, k,  z_l))) *(1.0d0- z_f)
             newrho(i,k,j)  = newrho(i,k,j) +  z_f*((rho(xn, k, z_l+1))*(1.0d0-x_f)+ x_f*(rho(xnp, k,z_l+1)))


#ifdef VELO

             newVtot(i,k,j)  = ((Vtot(xn, k, z_l)) * (1.0d0-x_f) + x_f*(Vtot(xnp, k,  z_l))) *(1.0d0- z_f)
             newVtot(i,k,j)  = newVtot(i,k,j) +  z_f*((Vtot(xn, k, z_l+1))*(1.0d0-x_f)+ x_f*(Vtot(xnp, k,z_l+1)))
  ...
  subroutine rotate_cube_azimuth(mu, phi, pivot, nx, ny, nz, dx, dy, dz)
   ...
   real(kind=8) phi , pi_n, sinphi, sintheta, cosphi 
   ...
   pivotdy = tan(phi) * pivot *sinphi  
   pivotdx = tan(theta) * pivot *cosphi 

   newdx = dz * sintheta *cosphi
   newdy = dz * sintheta *sinphi
